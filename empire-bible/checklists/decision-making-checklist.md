# Decision-Making Checklist
## For Major Empire Decisions

Use this checklist before making any decision involving capital allocation >$50,000, new business launches, major hiring, or strategic initiatives.

**Decision**: _______________

**Date**: _______________

**Decision Maker**: _______________

---

## Pre-Decision Phase

### 1. Information Gathering

- [ ] **Financial Data Collected**:
  - [ ] Revenue projections
  - [ ] Cost estimates
  - [ ] ROI calculations
  - [ ] Risk assessment
  - [ ] Cash flow impact

- [ ] **Strategic Data Collected**:
  - [ ] Alignment with 365-Year Vision
  - [ ] Multi-generational impact analysis
  - [ ] Dynasty Test evaluation
  - [ ] Strategic fit assessment

- [ ] **Legal/Compliance Data Collected**:
  - [ ] Legal requirements reviewed
  - [ ] Compliance implications assessed
  - [ ] Tax implications analyzed
  - [ ] Regulatory requirements identified

- [ ] **Family Input Collected**:
  - [ ] Family Council consulted (if required)
  - [ ] Key family members' opinions gathered
  - [ ] Family impact assessed
  - [ ] Family unity implications considered

---

### 2. Decision Matrix Completion

- [ ] Decision Matrix template completed
- [ ] All five criteria scored (Financial, Strategic, Values, Risk, Feasibility)
- [ ] Weighted total score calculated
- [ ] Score interpretation reviewed
- [ ] Recommendation determined

**Decision Matrix Score**: _______________/10

**Recommendation**: [ ] Approve [ ] Conditional [ ] Reject

---

### 3. Values Alignment Check

- [ ] 12 Pillars assessment completed
- [ ] All pillars evaluated for alignment
- [ ] No pillar violations identified
- [ ] Ethical boundaries checked
- [ ] Values Assessment Checklist completed (if major decision)

**Values Alignment**: [ ] ✅ Strong [ ] WARNING: Moderate [ ] ❌ Weak

---

### 4. Risk Assessment

- [ ] Financial risk evaluated
- [ ] Reputational risk evaluated
- [ ] Legal risk evaluated
- [ ] Family relationship risk evaluated
- [ ] Risk mitigation strategies identified
- [ ] Contingency plans developed

**Overall Risk Level**: [ ] Low [ ] Medium [ ] High

**Risk Acceptable**: [ ] Yes [ ] No

---

## Decision Phase

### 5. Authority Verification

**Decision Amount**: $_______________

**Required Authority Level**:
- [ ] Individual (<$10,000)
- [ ] Two Family Members ($10,000-$50,000)
- [ ] Family Council (>$50,000)
- [ ] Family Council + Trust Protector (>$500,000)

**Authority Obtained**: [ ] Yes [ ] No

**Approvals**:
- [ ] _______________ (Signature/Date)
- [ ] _______________ (Signature/Date)
- [ ] _______________ (Signature/Date)

---

### 6. Decision Documentation

- [ ] Decision documented in decision log
- [ ] Rationale recorded
- [ ] Assumptions documented
- [ ] Expected outcomes defined
- [ ] Success metrics identified
- [ ] Decision Matrix filed

**Documentation Location**: _______________

---

## Post-Decision Phase

### 7. Implementation Planning

- [ ] Implementation plan created
- [ ] Timeline established
- [ ] Key milestones identified
- [ ] Resources allocated
- [ ] Responsibilities assigned
- [ ] Success metrics defined

**Implementation Start Date**: _______________

**Expected Completion Date**: _______________

---

### 8. Communication

- [ ] Key stakeholders notified
- [ ] Family members informed (if applicable)
- [ ] Team members briefed (if applicable)
- [ ] Advisors updated (if applicable)
- [ ] Communication plan executed

**Stakeholders Notified**: 
- [ ] _______________
- [ ] _______________
- [ ] _______________

---

### 9. Monitoring Setup

- [ ] Monitoring schedule established
- [ ] Review dates set
- [ ] Success metrics tracking system created
- [ ] Reporting process defined
- [ ] Adjustment triggers identified

**Monitoring Schedule**:
- [ ] Weekly (first month)
- [ ] Monthly (first year)
- [ ] Quarterly (ongoing)

**First Review Date**: _______________

---

## Decision Review (Post-Implementation)

### 10. Outcome Evaluation

**Review Date**: _______________

**Actual Outcomes vs. Expected**:
- [ ] Met or exceeded expectations
- [ ] Partially met expectations
- [ ] Failed to meet expectations

**Success Metrics Results**:
- Metric 1: _______________ (Target: _______________)
- Metric 2: _______________ (Target: _______________)
- Metric 3: _______________ (Target: _______________)

**Lessons Learned**:
1. _______________
2. _______________
3. _______________

**Would Make Same Decision**: [ ] Yes [ ] No

**Reason**: 
_________________________________________________________________
_________________________________________________________________

---

## Decision Quality Assessment

**Decision Process Quality**: _______________/10

**Areas for Improvement**:
1. _______________
2. _______________
3. _______________

---

## Next Steps

- [ ] Implement decision (if approved)
- [ ] Monitor outcomes
- [ ] Adjust as needed
- [ ] Document lessons learned
- [ ] Update decision-making process if needed

---

**Checklist Complete**: [ ] Yes

**Completed By**: _______________

**Date**: _______________

---

**Next Steps**: 
1. Use this checklist for every major decision
2. Complete all phases before proceeding
3. Document everything
4. Review and learn from each decision
5. Improve process continuously
